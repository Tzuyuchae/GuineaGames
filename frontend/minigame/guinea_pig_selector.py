...
class GuineaPigSelector:
    ...
    def __init__(self, screen_width=672, screen_height=864, user_id=1, inventory_pigs=None):
        ...
        Args:
            ...
            inventory_pigs: List of GuineaPig objects from the local store inventory
        """
        self.screen_width = screen_width
        self.screen_height = screen_height
        self.user_id = user_id
        self.inventory_pigs = inventory_pigs # Store the passed inventory
    ...
    def _load_pets(self):
        """Load pets from local inventory, API, or fallback to mock data."""
        self.pets = []

        # 1. Check Local Inventory (Store Purchases)
        if self.inventory_pigs and len(self.inventory_pigs) > 0:
            print(f"Loading {len(self.inventory_pigs)} pigs from local inventory.")
            for pig_obj in self.inventory_pigs:
                # Convert the Breeding/Store object into a dict for the Selector
                pet_dict = {
                    'id': getattr(pig_obj, 'id', random.randint(1000,9999)),
                    'name': getattr(pig_obj, 'name', 'Unknown'),
                    'species': 'guinea_pig',
                    'color': 'brown', # Default if phenotype missing
                    'speed': 50,      # Default base stats
                    'health': 100,
                    'happiness': 100
                }

                # Try to extract specific genetics if available
                if hasattr(pig_obj, 'phenotype') and isinstance(pig_obj.phenotype, dict):
                    pet_dict['color'] = pig_obj.phenotype.get('coat_color', 'brown')
                
                # If the pig object has score (from store), maybe impact speed?
                if hasattr(pig_obj, 'score'):
                    # Map score 50-500 to speed 40-80
                    pet_dict['speed'] = int(40 + (pig_obj.score / 500) * 40)

                self.pets.append(pet_dict)

        # 2. If no local pigs, try API
        elif API_AVAILABLE:
            try:
                api = APIClient()
                api_pets = api.get_user_pets(self.user_id)
                if api_pets:
                    self.pets = api_pets
                else:
    ...
    def _create_pet_buttons(self):
        ...
        for i, pet in enumerate(self.pets):
            ...
            # Ensure the button logic uses top-left for positioning if your class works that way,
            # or center. Adjusting rect manually to be safe:
            button.rect.topleft = (start_x, y_pos)
    ...
    def draw(self, screen):
        ...
        if not self.pets:
            ...
            sub_text = self.info_font.render("Go to the Store to buy one!", True, WHITE)
            sub_rect = sub_text.get_rect(center=(self.screen_width // 2, 330))
            ...
            for i, (pet, button) in enumerate(zip(self.pets, self.pet_buttons)):
                ...
                color_val = pet.get('color', 'unknown')
                if isinstance(color_val, str):
                    color_val = color_val.title()
        ...
        if not self.selected_pet and self.pets:
